    ...
    def writeContent(self):
        ...
        if catdict and mapdict:
            ...
            for guid in catguids:
                ...
                wr('<tr><td>%s' % (catname))
                wr('<td>%s' % (rezcount))
                wr('<td><a href="Category_Edit?cat=%s" title="Edit"><i class="fa fa-pencil"></i></a>' % (catname))
                wr('<td>')
                if rezcount > 0:
                    pass
                else:
                    wr('<a href="Category_Delete?cat=%s" title="Delete"><i class="fa fa-trash"></i><a>' % (catname))
            wr('</table></P>')
        else:
